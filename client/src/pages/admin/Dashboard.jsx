...
export function AdminLayout({ children, title }) {
  ...
  return (
    <div dir={isRTL?'rtl':'ltr'} style={{ display:'flex', minHeight:'100vh', background:'#F8FAFB' }}>
      {/* Mobile bottom nav */}
      <nav className="admin-bottom-nav" style={{ justifyContent:'space-around', alignItems:'center' }}>
        {NAV.map(n => (
          <a key={n.path} href={n.path}
            style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:2, padding:'6px 8px', textDecoration:'none', color:location.pathname===n.path?'#1B5E20':'#888', fontSize:9, fontWeight:location.pathname===n.path?700:500, minWidth:40 }}>
            <span style={{ fontSize:20 }}>{n.icon}</span>
            <span style={{ whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth:50 }}>{n.label}</span>
          </a>
        ))}
      </nav>

      <aside className="admin-sidebar" style={{ width:220, background:'white', borderRight:'1px solid #f0f0f0', display:'flex', flexDirection:'column', padding:'20px 0', flexShrink:0 }}>
      ...
      <main className="admin-main" style={{ flex:1, padding:28, overflowY:'auto', paddingBottom:80 }}>
