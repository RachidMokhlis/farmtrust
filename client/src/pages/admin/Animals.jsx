  ...
  return (
    <AdminLayout title={lang==='ar'?'إدارة الحيوانات':lang==='fr'?'Gestion des animaux':'Animals Management'}>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
            style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:200, display:'flex', alignItems:'flex-end', justifyContent:'center', padding:'16px 8px' }}>
            <motion.div initial={{ scale:0.95, y:20 }} animate={{ scale:1, y:0 }} exit={{ scale:0.95 }}
              style={{ background:'white', borderRadius:'24px 24px 0 0', padding:'20px 16px', width:'100%', maxWidth:520, maxHeight:'92vh', overflowY:'auto' }}>
      ...
      {loading ? (
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(min(200px,45%),1fr))', gap:12 }}>
          {animals.map((a,i) => (
